 ...
 *     description: Obtiene una lista de todas los movimientos
 *     responses:
 *       '200':
...
exports.getAll = async (request, response) => {
    try {
        let movement = await Movement.findAll({
            order: [['id', 'desc']],
            raw: true
        });
 ...
 * /proyect/movement/:
 *   post:
 *     summary: Agregar un nuevo movimiento
 *     description: Agregar un nuevo movimiento con el nombre proporcionado.
 *     consumes:
 *       - application/json
 *     parameters:
 *       - in: body
...
exports.post = async (request, response)=>{ 
    ...
    try {   
        ...
        let save = await Movement.create(
            {
                name:name
            }
        );
